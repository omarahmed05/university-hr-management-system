...
{
    public partial class ViewStatusOfLeaves : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                SqlCommand statusLeavesFunc = new SqlCommand("SELECT * FROM dbo.status_leaves(@employee_ID)", conn);

                statusLeavesFunc.CommandType = CommandType.Text;

                statusLeavesFunc.Parameters.AddWithValue("@employee_ID", empID);

                conn.Open();

                SqlDataAdapter adapter = new SqlDataAdapter(statusLeavesFunc);
                DataTable dt = new DataTable();
                adapter.Fill(dt);

                leaveStatusGridView.DataSource = dt;
                leaveStatusGridView.DataBind();
                ...
                if (dt.Rows.Count == 0)
                {
                    ...
                    errorLabel.Text = "No leave requests found for this month.";
                }
            }
